...
class SpeedYUI:
    ...
    def Widgets(self):
        self.title = customtkinter.CTkLabel(self.master, text="Paste the URL here")
        self.title.pack(padx=10, pady=10)

        # Input Entry for URL
        self.link_variable = tkinter.StringVar()
        self.link = customtkinter.CTkEntry(self.master, width=480, height=30, textvariable=self.link_variable)
        self.link.pack(padx=10, pady=10)

        # Segment Button (MP3/MP4)
        self.datatype = customtkinter.CTkSegmentedButton(self.master, values=["MP3", "MP4"])
        self.datatype.pack()

        # Download Button
        self.download_button = customtkinter.CTkButton(self.master, text="Download", command=self.download, fg_color="red", hover="blue")
        self.download_button.pack(padx=10, pady=10)

        # Debug Label
        self.debug_label = customtkinter.CTkLabel(self.master, text="")
        self.debug_label.pack(padx=10)

        # Progress Bar (Currently not functional)
        self.progress_bar = customtkinter.CTkLabel(self.master, text="0")
